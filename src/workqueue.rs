...
use crate::{
    ...
    msg::MSGSubscriber,
    pthread::create_phtread,
};
use std::{
    any::Any,
    collections::{HashMap, VecDeque},
    ptr::null_mut,
    sync::{Arc, Condvar, LazyLock, Mutex, RwLock, Weak},
};
    ...
    */
    ...
    pub fn bind_msg_subscriber(self: &Arc<Self>, sub: &Arc<MSGSubscriber>) {
        sub.register_callback(
            Self::msg_subscriber_callback,
            Arc::into_raw(self.clone()) as *mut usize,
        );
    }
}
...
impl WorkQueue {
    pub fn new(
        ...
        let x = Arc::new_cyclic(|weak| {
            let _thread_id = create_phtread(
                stack_size,
                priority,
                workqueue_thread_handler,
                weak.as_ptr() as *mut libc::c_void,
                is_fifo_schedule,
            );
...
pub mod tests {
    ...
    impl GPS {
        fn run(ptr: *mut libc::c_void) {
            ...
            let gps = unsafe { &mut *(ptr as *mut Self) };
            gps.finish = true;
            println!("GPS is running!");
        }

        pub fn new(wq: &Arc<WorkQueue>) -> Arc<GPS> {
            let gps = Arc::new_cyclic(|gps_weak| {
    ...
    use crate::msg::{tests::*, MSGPublisher, MSG_LIST};
    #[test]
