        ...
        let entry = self.parent.upgrade().unwrap();
        {
            ...
            unsafe {
                std::ptr::copy_nonoverlapping(
                    data as *const T,
                    msg.data.as_mut() as *mut (dyn MessageMetaData + 'static) as *mut T,
                    1,
                );
            }
            msg.index += 1;
...
impl MSGSubscriber {
    ...
    pub fn get_latest<T: MessageMetaData>(self: &Arc<Self>) -> T {
        ...
        unsafe {
            ...
            std::ptr::copy_nonoverlapping(
                data.as_ref() as *const dyn MessageMetaData as *const T,
                ret.as_mut_ptr(),
                1,
            );
    ...
    pub fn register_callback<F>(self: &Arc<Self>, callback: F, arg: *mut usize)
    where
        F: FnMut(*mut usize) + 'static,
    {
...
pub mod tests {
    ...
    pub fn add_message_entry<T: MessageMetaData + 'static>(name: &'static str) {
        let list = &(*MSG_LIST) as *const HashMap<&str, Arc<MSGEntry>>
            as *mut HashMap<&str, Arc<MSGEntry>>;
